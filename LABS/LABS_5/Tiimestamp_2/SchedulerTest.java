package Napredno_Programiranje.LABS.LABS_5.Tiimestamp_2;
...
class Scheduler <T>{
    ...
    public Timestamp<T> next(){
        ...
        return timestamps.stream().sorted(Timestamp::compareTo).filter(t-> t.getTime().isAfter(localDateTime)).findFirst().orElse(null);
    }
    public Timestamp<T>last(){
        LocalDateTime localDateTime = LocalDateTime.now();
        return timestamps.stream().sorted(Timestamp::compareTo).filter(t-> t.getTime().isBefore(localDateTime)).reduce((i,j)->j).orElse(null);
    }
