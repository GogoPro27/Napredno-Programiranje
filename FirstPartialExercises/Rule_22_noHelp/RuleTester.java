package Napredno_Programiranje.FirstPartialExercises.Rule_22_noHelp;
//okej e interesna e , razglej ubavo za Optional i za Function i sto se dxesava u zadacata...
import java.util.*;
...
class RuleProcessor{
    public static <IN, OUT> void process(List<IN>inputs, List<Rule<IN, OUT>> rules) {
        inputs.stream().forEach(currInput -> {  System.out.println("Input: "+ currInput);
            rules.forEach(rule -> {
